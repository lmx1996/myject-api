...
// 客厅分类
router.get('/saloon', function (req, res, next) {
  var filter = { category:"客厅" }
  Good.find(filter, (err, doc) => {
    if (err) {
      res.json({
        status: '1',
        msg: err.message,
...
// 浴室分类
router.get('/showers', function (req, res, next) {
  var filter = { category:"浴室" }
  Good.find(filter, (err, doc) => {
    if (err) {
      res.json({
        status: '1',
